...
- `rtl_fm` est la commande permettant de démarrer l'enregistrement avec notre clé **SDR**. On lui passe comme argument : 
	...
	- `-s` -> La fréquence d'échantillonage en **kHz**. Dans notre cas, on capture **60 000** échantillons du signal chaque seconde. Et oui, le signal qui provient des satellites est analogique et comporte une infinité de valeurs, et ça nos **CPU** n'aiment pas, donc on vient capturer plusieurs points sur le signal afin de le transormer en un nombre **fini**. Plus on prendra d'échantillons, plus on aura un signal **numérique** fidèle à la réalité mais plus il sera lourd et long à traiter. **60k** est une bonne valeur pour conserver une bonne qualité sans avoir un fichier trop lourd.  
